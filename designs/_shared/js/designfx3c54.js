...
function createMobileMenu() {
    ...
    // If not, get from languages
    if (!itemColor) {
        itemColor = getElementColor($('#languages .mz_lngselect > ul > li > a'), 'color');
        itemSelectedColor = getElementColor($('#languages .mz_lngselect > ul > li.selected > a'), 'color');
        itemSelectedBackgroundColor = getElementColor($('#languages .mz_lngselect > ul > li.selected > a'));
    }
    ...
    '.mobile-header #menu ul li a { color: ' + itemColor + ' !important; background-color: transparent !important}' +
...
function setSlidingPanelColors(foregroundColor) {
    if ($('#sliding-panel-colors').length) return;
    ...
    css_output = css_output + '.sliding-panel-close { fill: ' + foregroundColor + ' } .sliding-panel .noUi-handle, .sliding-panel .noUi-connect { background-color: ' + foregroundColor + ' }';
...
function initFixedMenuColors() {
    ...
    // prepare color CSS
    var css_output =
    '#top.sticky:not(.legacy-sticky-menu), #top.sticky.legacy-sticky-menu #menu { background-color:#fff}';
...
/*function enableTransparentHeader() {      return;
    ...
    if (topSection.length && !topBanner.length) {
